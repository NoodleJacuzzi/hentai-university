var locationArray = [
	...
	{index: "sepia-schoolEntrance", name: "School Entrance", buttons: [
		{name: "Teacher's Lounge", top: 50, left: 65, type: "location", target: "sepia-teacherLounge", time: "MorningEvening",},
		{name: "Hallway", top: 79, left: 2, type: "location", target: "sepia-westHallway", time: "MorningEvening",},
		{name: "Gymnasium", top: 79, left: 70, type: "location", target: "sepia-gym", time: "MorningEvening",},
	],},
	{index: "sepia-westHallway", name: "West Hallway", buttons: [
		{name: "Classroom", top: 50, left: 70, type: "location", target: "sepia-classroomA", time: "MorningEvening",},
		{name: "School Entrance", top: 79, left: 35, type: "location", target: "sepia-schoolEntrance", time: "MorningEvening",},
	],},
	{index: "sepia-teacherLounge", name: "Teacher's Lounge", buttons: [
		{name: "Back to the Hallway", top: 79, left: 35, type: "location", target: "sepia-schoolEntrance", time: "MorningEvening",},
	],},
	{index: "sepia-classroomA", name: "Classroom A", buttons: [
		{name: "West Hallway", top: 30, left: 45, type: "location", target: "sepia-westHallway", time: "MorningEvening",},
	],},
	{index: "sepia-gym", name: "School Gym", buttons: [
		{name: "East Hallway", top: 39, left: 0, type: "location", target: "sepia-schoolEntrance", time: "MorningEvening",},
	],},
];
...
function changeLocation(n) {
	...
	if (locationTarget == 'failed') { //error message;
		...
		if (data.player.time == "Night" && data.player.location != "playerHouse") {
			if (data.player.day > 15 && data.player.carnivore != true && checkTrust("gyatrio") == 0) {
				var popularityScore = 0;
				if (checkTrust("kuro") > 1) {
					popularityScore += 1;
				}
				if (checkTrust("nikki") > 1) {
					popularityScore += 1;
				}
				if (checkTrust("ojou") > 1) {
					popularityScore += 1;
				}
				if (checkTrust("ribbon") > 1) {
					popularityScore += 1;
				}
				if (checkTrust("sports") > 1) {
					popularityScore += 1;
				}
				if (checkTrust("starlet") > 1) {
					popularityScore += 1;
				}
				if (checkTrust("president") > 1) {
					popularityScore += 1;
				}
				if (checkTrust("purple") > 1) {
					popularityScore += 1;
				}
				if (checkTrust("brown") > 1) {
					popularityScore += 1;
				}
				if (checkTrust("instructor") > 1) {
					popularityScore += 1;
				}
				if (checkTrust("meji") > 1) {
					popularityScore += 1;
				}
				if (checkTrust("nagatoro") > 1) {
					popularityScore += 1;
				}
				if (checkTrust("tomgirl") > 1) {
					popularityScore += 1;
				}
				console.log(popularityScore);
				if (popularityScore > 6) {
					if (checkFlag("mom", "pop3") != true) {
						document.getElementById('wrapperBG').style.backgroundImage = "url(images/gyatrio/profile.jpg)";
						loadEncounter("gyatrio", "intro3A");
					}
					else {
						returnHome();
					}
					...
					if (checkFlag("mom", "pop2") != true) {
						document.getElementById('wrapperBG').style.backgroundImage = "url(images/gyatrio/profile.jpg)";
						loadEncounter("gyatrio", "intro2");
					}
					else {
						returnHome();
					}
			...
			var flavoring = "";
			if (data.player.location.includes("sepia")) {
				flavoring = `style = "filter: sepia(1);"`;
				bg = bg.replace("sepia-", "");
			}
			document.getElementById('output').innerHTML += `
			...
			&& data.player.location.includes("sepia") != true
	...
	if (data.player.time == "Night") {
		var bg = "images/gyatrio/profile.jpg";
	}
	else {
