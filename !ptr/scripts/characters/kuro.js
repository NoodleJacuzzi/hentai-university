...
var logbook = { //Logbook details for each character.
	...
	artist: "Enoshima Iki<br>Original Image Set: Kuro Gal Bitch ga Uchi ni Kita!",
...
function writeEncounter(name) { //Plays the actual encounter.
	...
	for(var i = 0; i < data.story.length; i++)
		...
		case "kuro12" : {
			...
			writeFunction("writeEncounter('kuro13')", "Go for it", "blue");
			writeFunction("changeLocation(data.player.location)", "Hold off for now");
			break;
		}
		case "kuro13" : {
			document.getElementById('output').innerHTML = '';
			if(!checkFlag('kuro','endingA'))
				addFlag('kuro','endingA');
			if(!checkFlag('kuro','complete'))
				addFlag('kuro','complete');
			writeHTML(`
				sp player; Let's go for it, then. Might take a while for you to get into the swing of things, but it should be fine.
				t She smiles and nods.
				sp kuro; So, where do I start? I've probably got a pretty good handle on the whole 'sex' element, so... Paperwork?
				sp player; Hypnosis, actually.
				t She pauses, narrowing her eyes for a moment.
				sp kuro; Yeaaaah... Are you totally sure that's a real thing? Like, my head gets a bit hazy thinking about those other times, but like...
				t She thinks about her words for a moment.
				sp kuro; ...It just sounds like a bit much, y'know?
				sp player; You're probably just getting a little hung up on the name. I'll be using it extensively here, so my assistant really should know about some of my methods.
				sp kuro; Well... Okay. So, how does it work? If it always leaves me crazy forgetful, I'm not too sure I'll remember being convinced.
				sp player; Forgetting is optional, and let's discuss this at your place.
				t kuroF nods.
				sp kuro; Lead the way.
				...
				sp player; ...How much do you remember?
				t After getting to her home, you decided that the quickest and easiest way to handle convincing her was to pull everything from the previous sessions to her conscious mind.
				t Just doing that alone practically has her halfway into trance.
				sp kuro; Uh. I guess all of it? It felt... weird, but in a good way?
				t You nod at that, looking her in the eyes.
				sp player; Are you ready to try going under? You'll remember it all this time. No hazy recollections or half-formed memories.
				sp kuro; I...
				t She pauses to think for a moment, before nodding firmly.
				sp kuro; Yes. It's a bit weird to actually believe in hypnosis, but it's the same thing you've always been doing, so... whatev's. Put me under, doc!
				t You smile slightly, nodding and cupping her chin gently.
			`);
			writeFunction("writeEvent('kuro9')", "Put her all the way under");
			break;
		}
		case "kuro14" : {
			document.getElementById('output').innerHTML = '';
			writeHTML(`
				define kuro = sp kuro; im images/none.png;
				kuro Yo, Earth to *Master - you there?HIDDEN
				t You blink a few times and refocus, your eyes trailing along the figure of your assistant. She's been growing out her hair over the last year, but that's not what's-
				sp player; Ah, the scarf's new.
				im kuroOlderPic.jpg
				define kuro = sp kuro; im kuroOlder.jpg;
				t She smiles slightly, fiddling with the cloth.
				kuro Thanks for noticing, but that's not what we were talking about.
				sp player; Expanding our sphere of influence, yeah. Sorry, continue.
			`);
			if(checkTrust('maid') >= 200 || checkTrust('mistress') >= 200)
				writeHTML("kuro With the backing of mistressF's company, it wouldn't be too hard to start a bit of an international program.");
			else
				writeHTML("kuro Considering the funding we've got, it wouldn't be too hard to start a bit of an international program.");
			writeHTML(`
				sp player; Not sure how I feel about sending students <i>away</i> from the university, given what we do here.
				t She smirks slightly.
				kuro I meant accepting international transfers in. I'll leave the economic benefits of that sort of program to the people that paid attention in class, but getting a couple foreign cuties to enroll here would be pretty nice, don'tcha think?
				sp player; I certainly wouldn't have any complaints. Did you need my help for any of this, or...?
				kuro Nah, I'll just have pinstripeF handle it, and telling presidentF that you're interested will be enough to get her support too. I just wanted to make sure you signed off on it first.
				sp player; ...You've gotten pretty used to this position.
				t Her face goes a bit red at that, but she shoots back a smirk.
				kuro Well, you kept praising me whenever I did a good job, so that's really your fault, isn't it?
				t She runs her hand through her hair, flicking it back with confidence.
				kuro Oh, and before I forget, sportsF wanted to know if she could make some modifications to the cheerleader uniforms. The ones we actually use in public games, I mean.
				t You narrow your eyes slightly.
				sp player; What kind?
				kuro Just small vibrators for them to practice in, nothing too obvious. She sent me a link on the type, and I'd say they're pretty discreet. Plus, they're remote-controlled.
				sp player; Hm... Have her order a few and we can test them out. I want to see how 'discreet' they are for myself before anything else.
				t kuroF gives you a smirk at that, before raising her skirt and using her thumb to pull down her panties.
				t No longer blocked by the cloth, you can see the small black toy pressing into her pussy, blocking the view of her cunt and clit as it vibrates gently.
				sp player; ...Approved.
				kuro Perfect~ She'll be over the moon about it. I'll make sure you can control 'em with your phone so you can change the intensity yourself.
				t With that, she turns around with a slight spin, before smiling back at you.
				t Her thumb teases down her panties for a moment, showing off a buttplug.
				kuro And I'll make sure you can control this one too~
				sp player; Keep teasing me like that, kuroF, and neither of us'll be leaving this room any time soon.
				kuro Don't threaten me with a good time, *Master~
				t She winks to you, before fixing her panties and opening the door.
				kuro Oh, and you have a meeting in an hour with secretaryF and principalF.
				sp player; I remember, yeah.
				kuro Cool cool. And we're still on for dinner tonight?
				sp player; Of course, but I'd better have control of those toys of yours by then.
				kuro I was hoping you'd want that~
				t She gives you a wink before shutting the door behind her, and you lean back with a sigh and a bit of a smile.
				t Seems like things are set to be going well for a while. School under your thumb, a capable and <b>highly</b> fuckable assistant helping out with deskwork (including under-the-desk-work), and pretty soon a new flock of students...
				t Everything's going perfectly, and will be for a while.
			`);
			writeFunction("loadEncounter('system', 'credits')", "The End");
			break;
		}
...
function writeEvent(name) { //Plays the actual event.
	...
	switch (name) {
		case "kuro1" : {
			...
			writeText("You quickly shimmy down your pants and underwear, though you see "+fName('kuro')+" starting to fidget less-than-discreetly with her phone.");
		...
		case "kuro9" : {
			writeHTML(`
				t It doesn't take long at all for her to go under, not with the fun you've had with her in the past. She might be having a bit of trouble consciously accepting it, but subconsciously...
				t Well, her eyes are still somewhat focused as you speak, but it doesn't take a genius to see that the lights might be on, but no one's at the door.
				sp player; Let's go ahead and stop worrying about those doubts for now, kuroF. We don't have to, since it'd be <i>so easy</i> to solve them later, right?
				t She nods slightly.
				sp player; Instead, let's focus on something <i>better.</i> Let's focus on <i><b>pleasure.</b></i>
				t There's a slow, shuddering breath from her at that, but she nods again.
				t You raise a hand to her lips, running your thumb along them before teasing her mouth open with it.
				sp player; You're going to feel very, <i>very</i> good tonight. With every touch of my skin against yours, and with every moan I make from you pleasing <i>me,</i> you'll feel better and better.
				t Your thumb toys gently with her tongue, which shifts around and teases at your thumb almost like it were a cock.
				sp player; And that pleasure will keep rising, and rising, and rising. Cumming won't make it stop, and it certainly won't make it go down. 
				t Your other hand teases along her ass as you whisper into her ear, your fingers gently brushing against her panties.
				sp player; And as it keeps rising, you'll keep falling deeper and deeper into that pleasure, wanting to <i>share</i> that pleasure and feeling even better when you do. And, of course...
				t You speak even softer now, directly into her ear as you tease her pussy through the cloth.
				sp player; Every time I call you a <b>good girl,</b> that pleasure will <i><b>spike.</b></i> After all, it feels good to be praised by your *Master, doesn't it?
				sp kuro; Y-Yesh~...
				sp player; <b>Good girl.</b>
				t She shudders again, her face flushed red now.
				sp player; Then let's go ahead and get started, hm? In a few moments, you'll come back from zoning out, ready for your pleasure to keep rising. You'll remember this bit of conditioning and, of course, you'll be all the more willing to believe everything I say. Does that sound good?
				t You pull your thumb out of her mouth to let her reply.
				sp kuro; Yes, *Master.
				sp player; Then be a <b>good girl</b> and focus on me again.
				t She blinks once, before smiling at you and shaking her head gently.
				sp kuro; Haah... That felt a bit weird. Kinda like a dream, almost.
				t Saying that, her hand moves to play with the bulge in your clothes, focusing on you again.
				sp kuro; But I can worry about that hypnosis thing later. For now, how about you show me that delicious fuckstick of yours so I can get started, *Master~?
				...
				t kuroF gives you a coy wink as her fingertips slowly stroke up and down your shaft.
				im 11-1.jpg
				sp kuro; Enjoying the view, hun~? 
				t Your cock throbs in her hand.
				sp player; I'd enjoy it more if you put that mouth to work.
				t She smirks back at you, but quickly brings her lips to your shaft.
				t Her mouth quickly takes your head in, her hand starting to move quicker as strokes you into her mouth.
				sp kuro; Mm~
				t You can seem her smile a bit, even with her lips wrapped around your cock, as she raises her head up.
				t Only the tip is still in, before she starts working her tongue too.
				im 11-2.jpg
				sp player; Nn... That's better.
				t Her lips separate from your cock completely as she smiles, a sultry look in her eyes as her head goes down.
				t She slowly peppers your shaft with kisses as she goes, until she reaches your balls.
				t Slowly teasing them with her tongue and lips, her hand keeps stroking your shaft as you take a deep, pleased breath.
				sp kuro; Hah~ Enjoying this, *Master~?
				t You just let out a deep exhale in response, which seems like more than enough for her.
				t Sticking out her tongue, she slowly teases your shaft all the way from the base to the tip, before taking you into her mouth again.
				t This time, she doesn't stop at the head, though.
				t She dips down further, her mouth taking half your shaft as she shifts her head slightly, rotating around your cock as her tongue continues its teasing.
				sp player; Fuck... <b>Good girl...</b>
				t You feel her shudder against you as she hears it, a soft moan muffled by you cock as she starts going faster.
				t One of her hands goes to tease at her slit as she bobs up and down, her eyes shutting gently at times as she savors every inch, and often opening back up to make eye-contact while trying to please you.
				t Your own hand moves to shift some of her hair from her face, her bobbing pausing to let you before resuming even faster.
				sp player; I'm getting close.
				t She gives a very small nod, hearing her inhale sharply... before taking you into her throat.
				sp kuro; Glrk~
				t It feels as tight and wet around you as before, the wet sound of her taking you balls deep ringing out.
				t A second later, she rises back up, her mouth tight around your shaft as you feel her moan.
				t Her mouth shifts a bit now as she rises, the sensation of a vacuum-blowjob catching you off-guard for a moment as she bobs up and down the upper-half of your shaft a few times.
				t It only lasts a moment, though, before you're balls-deep in her throat again, getting closer and closer to the edge.
				t The only warning you give her is a low groan of deep pleasure, leaning your head back as you spurt out ropes of cum into her mouth.
				im 11-3.jpg
				t The light suction of her mouth is joined by her hand milking out every drop she can, swallowing each spurt with a smile at first, before winking slightly and pulling her mouth away.
				t The rest of your cum shoots out across her face and chest, as well as back onto your abdomen. Some drops down onto your shaft as she keeps stroking, her eyes still on yours.
				sp kuro; Ahhh~
				im 11-4.jpg
				t kuroF wiggles her tongue around a bit, almost letting the cum drip down from it, before she pulls in back into her mouth for a deep swallow.
				sp kuro; Mm, there we go~
				t Kissing the head of your shaft again, she brings her cum-covered hand down to her pussy with a wry smile.
				sp kuro; D'ya think that's enough <i>lube,</i> *Master~?
				sp player; It just might be, slut.
				t Reaching forward, you pull her towards you firmly, her giggling filling the room as you do.
				sp player; Even like this, you still act so <i>spirited,</i> you know that?
				t She raises an eyebrow at you, even as she rolls on the bed a bit to get into a better position.
				sp kuro; Would you prefer a mind-broken sex doll? 'Cause I wouldn't mind getting the whole 'fucked stupid' treatment again~
				t With kuroF on her back, you lean over her and grab her legs. Your shaft presses against her pussy, gliding along it as she shudders and bits her lip.
				sp player; You "wouldn't mind"? Now that doesn't sound enthusiastic at all.
				t Rolling your hips, you press the head of your shaft against her entrance... before letting it slide along her pussy instead.
				sp kuro; F-Fuck...!
				sp player; I think you can come up with a better way to beg, don't you?
				sp kuro; Y-Yes~ I want to you to fuck me, *Master!
				sp player; That <i>still</i> doesn't sound right.
				t You grab your cock by the base, lightly slapping it down against her cunt a few times as kuroF bites her lip.
				sp player; If you want this, then <i>beg.</i>
				t It takes a moment for her to take a long, deep breath, before she gives you a lustful look and shouts,
				sp kuro; Use me like a warm, whimpering Fleshlight and dump your cum into my tan little cunny, *Master~!
				sp player; <b>Good girl.</b>
				sp kuro; <i><b><font size='+2'>AHHN~</font></b></i>
				im 11-5.jpg
				t Your cock pushes deep inside of her pussy as she shakes gently underneath you, the pleasure continuing to rise as you shift and pitch your hips forward and back.
				t From her expression, it's not hard to tell that the pleasure's building fast.
				sp player; You're absolutely <i>soaked</i> down here, kuroF.
				t You move your hand to play with her chest for a moment, watching her squirm and bite her lip as you do.
				sp player; Were you that desperate to get used like a sex-toy, or was sucking me off just too stimulating for you?
				sp kuro; B-Both...! Nn, fuck~ Just tasting your pre had me so wet, and when I was throating you-
				t You interrupt her with a heavy thrust, her toes curling slightly as her pussy tightens around you.
				sp kuro; I th-thought I might cum from feeling you so d-deep~!
				t Another sharp inhale, followed by her eyes rolling up a bit as she keeps trying to focus.
				t Her cunt keeps squeezing tight around you, her wet folds feeling incredible as she squeezes in time with your thrusts.
				sp player; And how about now? How close to cumming are you?
				t kuroF's eyes go to yours as she squeezes tight around your shaft again.
				sp kuro; I'm still... Even right now, I haven't stopped~
				t Another heavy thrust accompanies the rest, her eyes squeezing tight as she gasps.
				sp kuro; But you said to f-focus on you, so I'm trying...! I'm trying really, <i>really</i> hard~
				sp player; ...Cute.
				t You use your thumb to gently tease at her clit, smiling down at her even as the pleasure causes her to squirm more.
				sp player; Go ahead, then. Focus on me, and only me. No matter how good you feel, no matter how many times you cum... You'd better stay awake.
				t You move your hand back to her leg again, picking up speed and aiming for her G-spot with every thrust.
				sp player; You passed out before I finished last time, but if you want to be my assistant, then you'd better be able to keep up.
				sp kuro; Y-Yes, *Master...! I promise, I won't disappoint~
				...
				sp player; Fuck, here we go...!
				t The last of your cum spills into her as you let out a heavy sigh.
				sp kuro; Th-thank you, *Master...
				t She's practically limp on the bed, breathing heavily as you lay down beside her.
				t Given your orders, she's probably barely conscious from the sensations coursing through her right now, so...
				sp player; Let's go ahead and let all that pleasure drain out.
				t You press your lips to hers, and a full-body shudder shakes through for just a moment, before she sinks back down into the bed.
				t You're not sure you've ever heard a sigh as completely relaxed as the one she lets out, her hand resting gently against her forehead.
				sp kuro; I feel...
				t Her chest rises as she takes a deep breath, the cum on her tits sliding a bit.
				sp kuro; I feel like I could sleep for a long fuckin' time~...
				t She lets out a short giggle at that, before going back to simply breathing.
				im 11-6.jpg
				t You're not entirely sure how long you're just laying there watching her breathe, but eventually you speak up.
				sp player; You did a good job, it felt great.
				sp kuro; H-Hah... Felt like I was a limp fuck-toy for most of it hun, but I'll take the praise~
				t She shifts slightly, opening an eye to look at you.
				im 11-7.jpg
				sp kuro; Next time... Hah, well, maybe not <i>next-time</i> next time, but...
				t She looks up to the ceiling contentedly.
				sp kuro; I'm gonna <i>more</i> than just keep up with you, hun. Then I'll be the one calling <i>you</i> cute~
				t You can't help but laugh a bit at that.
				sp player; We'll see about that. There's all sorts of triggers out there that might make that a bit difficult for you, y'know.
				sp kuro; Mm, that'll just make the challenge of it all the more fun~ You've got all the time in the world now, after all, to spend <i>training</i> your lovely assistant~!
				t Her eyes shut again at that as she smiles.
				sp kuro; But, for now... Maybe sleep.
				t She gently grabs your hand and pulls you close to her, breathing deeply into your chest.
				t You're feeling pretty tired yourself after all that, so you go along with it.
				t The clean-up on all this will be a fucker to deal with, but that can wait for tomorrow.
				t It's time to rest for quite a while...
			`);
			if(data.player.location == 'gallery')
				writeFunction("changeLocation(data.player.location)", "Finish");
			else
				writeFunction("writeEncounter('kuro14')", "Sleep");
			break;
		}
	}
...
var phoneArray = [//Lists the potential text events the player can receive at the start of the day, depending on their trust.
	...
	{index: "kuroReward1", requirements:"?trust kuro 999;"},
...
switch (requestType) {
	case "test": {
			//writeText("test");
			addEncounter("kuro", "statusQuo", "Enter Midnight Bliss")
